# 📖 Biblia - Telegram Bot Project

Проект состоит из двух независимых Telegram ботов для работы с библейским контентом и технической поддержкой. Боты работают на Python с использованием aiogram 3.x, PostgreSQL и OpenAI API.

[![Python](https://img.shields.io/badge/Python-3.11+-blue.svg)](https://www.python.org/)
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-15+-blue.svg)](https://www.postgresql.org/)
[![aiogram](https://img.shields.io/badge/aiogram-3.10+-blue.svg)](https://docs.aiogram.dev/)
[![OpenAI](https://img.shields.io/badge/OpenAI-API-green.svg)](https://openai.com/)

## 🚀 Быстрый старт

Подробная инструкция по развертыванию: **[DEPLOYMENT.md](DEPLOYMENT.md)**

```bash
# 1. Клонируйте репозиторий
git clone https://github.com/bah677/Biblia.git
cd Biblia

# 2. Настройте базу данных
cd database && sudo ./setup_database.sh && cd ..

# 3. Создайте .env файл из примера
cp .env.example .env
nano .env  # Заполните все переменные

# 4. Установите зависимости и запустите
cd user_bot && pip install -r requirements.txt && cd ..
cd admin_bot && pip install -r requirements.txt && cd ..

# 5. Запуск через supervisor (рекомендуется) или напрямую
python user_bot/main.py &
python admin_bot/main.py &
```

## 🤖 Боты

### User Bot - Основной бот для пользователей
- **Функции:**
  - `/start` - Приветствие и работа с OpenAI Assistant
  - `/more` - 13 тем из Писания с поддержкой ИИ
  - `/support` - Создание тикетов поддержки
  - `/mytickets` - Просмотр своих тикетов
  - `/affiliate` - Реферальная система

### Admin Bot - Панель администратора
- **Статистика:**
  - `/stats` - Общая статистика бота
  - `/token_stats` - Статистика токенов OpenAI
  - `/token_leaderboard` - Топ пользователей
  
- **Работа с тикетами:**
  - `/tickets` - Список активных тикетов
  - `/my_tickets` - Мои взятые тикеты
  - `/viewTKT...` - Просмотр конкретного тикета
  - Взятие в работу, ответ, закрытие тикетов
  
- **Управление:**
  - `/add_admin` - Добавить администратора
  - `/remove_admin` - Удалить администратора
  - `/list_admins` - Список администраторов

## 🏗️ Архитектура

```
/app/
├── user_bot/          # Основной бот для пользователей
│   ├── app/
│   │   ├── bot/       # Логика бота
│   │   └── helpers/   # Вспомогательные модули
│   ├── main.py
│   ├── config.py
│   ├── requirements.txt
│   └── .env.example
│
├── admin_bot/         # Админский бот
│   ├── app/
│   │   └── bot/       # Логика админского бота
│   ├── main.py
│   ├── config.py
│   ├── requirements.txt
│   └── .env.example
│
├── shared/            # Общие модули (используются обоими ботами)
│   ├── storage/       # Общая работа с БД
│   │   ├── database.py
│   │   ├── user_storage.py
│   │   ├── ticket_storage.py
│   │   └── content_storage.py
│   └── models/        # Модели данных
│
├── database/          # SQL скрипты для инициализации БД
│   ├── 01_init_database.sql
│   ├── 02_create_tables.sql
│   ├── 03_insert_initial_data.sql
│   └── setup_database.sh
│
├── docs/              # Документация проекта
│
├── supervisor_bots.conf  # Конфигурация Supervisor
└── DEPLOYMENT.md      # Полная инструкция по развертыванию
```

## 🛠️ Технологии

- **Python 3.11+**
- **aiogram 3.10+** - Telegram Bot Framework
- **OpenAI API** - GPT-4 для ответов
- **PostgreSQL** - База данных
- **asyncpg** - Асинхронная работа с PostgreSQL
- **Supervisor** - Управление процессами

## 📦 Установка

### 1. Клонирование репозитория

```bash
git clone https://github.com/bah677/Biblia.git
cd Biblia
```

### 2. Установка зависимостей

```bash
# User Bot
cd user_bot
pip install -r requirements.txt

# Admin Bot
cd ../admin_bot
pip install -r requirements.txt
```

### 3. Настройка PostgreSQL

**Автоматическая установка (рекомендуется):**

```bash
cd database
chmod +x setup_database.sh
sudo ./setup_database.sh
cd ..
```

Скрипт автоматически:
- Установит PostgreSQL (если нужно)
- Создаст пользователя `bot_user`
- Создаст базу данных `telegram_bot`
- Создаст все таблицы с правильными правами
- Загрузит начальные данные
```

### 4. Настройка единого .env файла

**Создайте один .env файл в корне проекта:**

```bash
cp .env.example .env
nano .env  # Заполните своими данными
```

**Преимущества единого .env:**
- ✅ Все настройки в одном месте
- ✅ Общие переменные (БД) не дублируются
- ✅ Проще управлять и обновлять
- ✅ Легче развертывать на сервере

Подробнее: [ENV_CONFIGURATION.md](ENV_CONFIGURATION.md)

### 5. Запуск через Supervisor

```bash
# Копирование конфигурации
sudo cp supervisor_bots.conf /etc/supervisor/conf.d/bots.conf

# Обновление supervisor
sudo supervisorctl reread
sudo supervisorctl update

# Запуск ботов
sudo supervisorctl start bots:*
```

## 🔑 Получение токенов

### Telegram Bot Token
1. Откройте [@BotFather](https://t.me/BotFather)
2. Отправьте `/newbot`
3. Следуйте инструкциям
4. Скопируйте токен

### OpenAI API Key
1. Зайдите на [platform.openai.com](https://platform.openai.com)
2. Перейдите в API Keys
3. Создайте новый ключ
4. Скопируйте и сохраните

### OpenAI Assistant ID
1. Зайдите на [platform.openai.com/assistants](https://platform.openai.com/assistants)
2. Создайте нового ассистента с инструкциями
3. Скопируйте ID ассистента

## 🗄️ База данных

Проект использует PostgreSQL со следующими таблицами:

- `users` - Пользователи
- `messages` - История сообщений
- `support_tickets` - Тикеты поддержки
- `admins` - Администраторы
- `token_usage` - Статистика использования токенов
- `bot_content` - Контент для кнопок
- `referrals` - Реферальная система

## 🔄 Workflow тикетов

1. Пользователь создает тикет → User Bot
2. Сохранение в БД
3. Уведомление в админскую группу → Admin Bot
4. Админ берет в работу (сообщение редактируется)
5. Админ отвечает пользователю
6. Админ закрывает тикет (сообщение удаляется)

## 📊 Управление

### Команды Supervisor

```bash
# Статус
sudo supervisorctl status bots:*

# Перезапуск
sudo supervisorctl restart bots:*

# Остановка
sudo supervisorctl stop bots:*

# Логи
tail -f /var/log/supervisor/user_bot.out.log
tail -f /var/log/supervisor/admin_bot.out.log
```

## 🧪 Тестирование

1. Создайте тикет в User Bot через `/support`
2. Проверьте что уведомление пришло в админскую группу
3. Откройте Admin Bot и попробуйте команды
4. Возьмите тикет в работу и проверьте обновление статуса

## 🔐 Безопасность

⚠️ **ВАЖНО:**
- Никогда не публикуйте `.env` файлы в Git
- Держите токены в секрете
- Регулярно обновляйте API ключи
- Используйте сильные пароли для БД

## 📝 Лицензия

Этот проект создан для личного использования.

## 👤 Автор

GitHub: [@bah677](https://github.com/bah677)

## ✨ Ключевые особенности проекта

### 1. Единый файл конфигурации
- **Один `.env` файл** в корне для обоих ботов
- Упрощает развертывание и управление
- Общие переменные (БД) не дублируются

### 2. Правильные права доступа PostgreSQL
- Автоматическое назначение `bot_user` владельцем таблиц
- Полная документация в [docs/DATABASE_PERMISSIONS.md](docs/DATABASE_PERMISSIONS.md)
- Решение типичных проблем с правами

### 3. Интеграция между ботами
- User Bot создает тикеты → уведомляет Admin Bot
- Admin Bot постит в группу с deep link
- Автоматическое обновление/удаление сообщений при изменении статуса

### 4. Полная документация
- 📘 [README.md](README.md) - Общая информация и быстрый старт
- 📗 [DEPLOYMENT.md](DEPLOYMENT.md) - Подробное развертывание
- 📕 [QUICK_DEPLOY.md](QUICK_DEPLOY.md) - Чеклист на 10 минут
- 📙 [ENV_CONFIGURATION.md](ENV_CONFIGURATION.md) - Конфигурация переменных
- 📓 [docs/DATABASE_PERMISSIONS.md](docs/DATABASE_PERMISSIONS.md) - Права БД

## 🆘 Troubleshooting

### Ошибка "must be owner of table"
```bash
# Назначьте bot_user владельцем
sudo -u postgres psql -d telegram_bot << 'EOF'
ALTER TABLE users OWNER TO bot_user;
ALTER TABLE admins OWNER TO bot_user;
ALTER TABLE bot_content OWNER TO bot_user;
ALTER TABLE messages OWNER TO bot_user;
ALTER TABLE referrals OWNER TO bot_user;
ALTER TABLE support_tickets OWNER TO bot_user;
ALTER TABLE token_usage OWNER TO bot_user;
EOF
```

### Бот не запускается
```bash
# Проверьте логи
tail -f /var/log/supervisor/*.log

# Проверьте PostgreSQL
sudo systemctl status postgresql

# Проверьте .env файл
cat .env | grep -v "^#" | grep "="
```

### Проблемы с подключением к БД
```bash
# Тест подключения
psql -U bot_user -d telegram_bot -h localhost -W

# Проверка прав
sudo -u postgres psql -d telegram_bot -c "\dt"
```

Подробное решение проблем: [QUICK_DEPLOY.md](QUICK_DEPLOY.md#-troubleshooting)

---

📚 **Документация:** См. папку [docs/](docs/) для дополнительных гайдов

⭐ **Если проект полезен, поставьте звезду!**
